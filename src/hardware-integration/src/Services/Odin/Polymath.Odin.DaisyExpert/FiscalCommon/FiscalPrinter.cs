...
namespace Polymath.Odin.DaisyExpert.FiscalCommon
{
    ...
    public class FiscalPrinter
    {
        ...
        public string infoLastErrorText = "";
        ...
        public void Connect()
        {
           ...
           // Thread.Sleep(500);
                ...
                //needed for bluetooth to wake up
                Thread.Sleep(200);
    ...
    0x0444, 0xF4, 0x0445, 0xF5, 0x0446, 0xF6, 0x0447, 0xF7, 0x0448, 0xF8, 0x0449, 0xF9, 0x044A, 0xFA, 0x044B, 0xFB, 0x044C, 0xFC, 0x044D, 0xFD, 0x044E, 0xFE, 0x044F, 0xFF, };
        ...
        void WritePacket(int cmd, string arguments)
        {
            ...
            for (int retry = 0; retry < MAX_RETRIES; retry++)
            {
                ...
                {
                    b = comm.ReadByte();
                } while (b == 0x16 && (DateTime.Now - startTime).TotalSeconds < 60);
        ...
        public string CheckResult()
        {
            ...
            {
                if ((StatusBytes[0] & (1 << 6)) != 0) infoLastErrorText += "Cover is open.\n";
            }
            if ((StatusBytes[0] & (1 << 5)) != 0) infoLastErrorText += "General error - OR of all errors marked with #.\n";
            if ((StatusBytes[0] & (1 << 4)) != 0) infoLastErrorText += "# Failure in printing mechanism\n";
            if ((StatusBytes[0] & (1 << 2)) != 0) infoLastErrorText += "# The real time clock is not synchronized\n";
            if ((StatusBytes[0] & (1 << 1)) != 0) infoLastErrorText += "# Invalid command code\n";
            if ((StatusBytes[0] & (1 << 0)) != 0) infoLastErrorText += "# Incoming data has syntax error\n";
            if ((StatusBytes[1] & (1 << 6)) != 0) infoLastErrorText += "The built-in tax terminal is not responding\n";
            if ((StatusBytes[1] & (1 << 3)) != 0) infoLastErrorText += "Low battery (the real-time clock is in RESET status)\n";
            if ((StatusBytes[1] & (1 << 2)) != 0) infoLastErrorText += "The RAM has been reset\n";
            if ((StatusBytes[1] & (1 << 1)) != 0) infoLastErrorText += "# Command is not permitted.\n";
            if ((StatusBytes[1] & (1 << 0)) != 0) infoLastErrorText += "#Overflow during command execution.\n";
            if ((StatusBytes[2] & (1 << 2)) != 0) infoLastErrorText += "# EJ is full\n";
            if ((StatusBytes[2] & (1 << 0)) != 0) infoLastErrorText += "# Out of paper\n";
            if ((StatusBytes[4] & (1 << 5)) != 0) infoLastErrorText += "OR of all mistakes marked by *\n";
            if ((StatusBytes[4] & (1 << 4)) != 0) infoLastErrorText += "* Fiscal memory is full\n";
            if ((StatusBytes[4] & (1 << 0)) != 0) infoLastErrorText += "* Fiscal memory write error\n";
            //if ((StatusBytes[5] & (1 << 5)) != 0) infoLastErrorText += " Fiscal memory read error\n";
            if ((StatusBytes[5] & (1 << 2)) != 0) infoLastErrorText += "* The last fiscal memory store operation is not successful\n";
            if ((StatusBytes[5] & (1 << 0)) != 0) infoLastErrorText += "* The fiscal memory is set in READONLY mode (locked)\n";
            if (infoLastErrorText.Length > 0)
                throw new FiscalException(-23, infoLastErrorText);

            return infoLastErrorText;
        }

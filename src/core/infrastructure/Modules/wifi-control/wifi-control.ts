let CXT, WiFiScanner, execSyncToBuffer, os_instructions;

WiFiScanner = require('../node-wifiscanner2/wifiscanner');
...
switch (process.platform) {
  case "linux":
    os_instructions = require('./linux').default;
    break;
  case "win32":
    os_instructions = require('./win32').default;
    break;
  case "darwin":
    os_instructions = require('./darwin').default;
    break;
  default:
    ...
    try {
      ...
      check_iface = (function (_this) {
        return function (_ap, cb) {
          ...
          if (((new Date() as any) - t0) < CXT.WiFiControlSettings.connectionTimeout) {
